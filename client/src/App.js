...
function About() {
  return (
    <section id="about" style={{ display: 'flex', alignItems: 'center', padding: 60, backgroundColor: '#f9f9f9' }}>
      <div style={{ flex: 1, paddingRight: 40 }}>
        <h2 style={{ fontSize: 32, color: '#184C61' }}>About Us</h2>
        <p style={{ fontSize: 18, lineHeight: 1.6 }}>
          Our platform is a powerful centralized identity and registration system built to help organizations
          manage their internal users through secure, private dashboards. Each company gets its own isolated
          database and admin panel, allowing complete control over user data — from registration to verification.
          <br /><br />
          We provide a modern, single-page application (SPA) interface powered by React, Node.js, and MySQL to ensure
          fast, secure, and scalable performance for any organization — schools, companies, or institutions.
        </p>
      </div>
      <div style={{ flex: 1 }}>
        <img src={drAbeyahmed} alt="About Us" style={{ width: '100%', maxWidth: 500, borderRadius: 10 }} />
      </div>
    </section>
  );
}
function Service() {
  return (
    <section id="services" style={{ display: 'flex', alignItems: 'center', padding: 60, backgroundColor: '#fff' }}>
      <div style={{ flex: 1 }}>
        <img src={nidImg} alt="Our Services" style={{ width: '100%', maxWidth: 500, borderRadius: 10 }} />
      </div>
      <div style={{ flex: 1, paddingLeft: 40 }}>
        <h2 style={{ fontSize: 32, color: '#184C61' }}>Our Services</h2>
        <ul style={{ fontSize: 18, lineHeight: 1.8 }}>
          <li><strong>Company Registration:</strong> Quick signup and instant access to a private dashboard and database.</li>
          <li><strong>Super Admin Dashboard:</strong> Full control over all companies, system settings, and activity monitoring.</li>
          <li><strong>User Registration:</strong> Company admins can add internal users with detailed information.</li>
          <li><strong>Identity Verification:</strong> Verify users using Fayda ID or Full Name from internal or mock databases.</li>
          <li><strong>Authentication:</strong> Secure login via username/Gmail with password reset features.</li>
          <li><strong>Real-Time SPA Experience:</strong> Smooth navigation, dynamic transitions, and user-friendly UI.</li>
          <li><strong>Image/File Uploads:</strong> Upload company logos and user profile photos with ease.</li>
        </ul>
      </div>
    </section>
  );
...
function SuperAdminDashboard() {
  ...
  const [profile, setProfile] = useState({ username: 'superadmin', password: '', newPassword: '', profileImg: null });
  ...
  useEffect(() => { fetchCompanies(); fetchActivity(); }, []);
  ...
  return (
    <section className="super-admin-dashboard">
      ...
      <p className="super-admin-desc">Full system control: register, view, update, delete companies, reset passwords, monitor activities.</p>
      ...
      <form className="admin-profile-form" onSubmit={handleProfileSave}>
        ...
        <div className="profile-fields-col">
          ...
          <div className="profile-field">
            ...
            <input type="password" name="password" placeholder="Enter current password" value={profile.password} onChange={handleProfileChange} />
          </div>
          <div className="profile-field">
        ...
        <table className="admin-company-table">
          ...
          <tbody>
            {filteredCompanies.map(c => (
              <tr key={c.id}>
                <td>{editId === c.id ? <input name="company_name" value={editForm.company_name} onChange={handleEditChange} /> : c.company_name}</td>
                <td>{editId === c.id ? <input name="username" value={editForm.username} onChange={handleEditChange} /> : c.username}</td>
                <td>{editId === c.id ? <input name="gmail" value={editForm.gmail} onChange={handleEditChange} /> : c.gmail}</td>
                <td>{editId === c.id ? <input name="phone" value={editForm.phone} onChange={handleEditChange} /> : c.phone}</td>
                <td>{editId === c.id ? <input name="category" value={editForm.category} onChange={handleEditChange} /> : c.category}</td>
                <td>
      ...
      <ul className="admin-activity-log">
        ...
        {activity.map((a, idx) => (
          <li key={idx}>
            <span className={`activity-type activity-type-${a.type}`}>{a.type.replace('-', ' ').toUpperCase()}</span>
            <span className="activity-details">{JSON.stringify(a.details)}</span>
...
function Contact() {
  return (
    <section id="contact" style={{ display: 'flex', alignItems: 'center', padding: 60, backgroundColor: '#f4f8fb' }}>
      <div style={{ flex: 1, paddingRight: 40 }}>
        <h2 style={{ fontSize: 32, color: '#184C61' }}>Contact Us</h2>
        <p style={{ fontSize: 18, lineHeight: 1.6 }}>
          Get in touch with our team for any questions about our identity management platform.
          We're here to help you implement secure, reliable solutions for your organization.
        </p>
        <div style={{ marginTop: 20 }}>
          <p><strong>Email:</strong> info@faydapass.com</p>
...
function App() {
  ...
  return (
    <div className="App" style={appBgStyle}>
      ...
      <div className={`main-content${!isDashboardRoute ? ' content-with-bg' : ''}`}>
      <Routes>
        ...
        <Route path="/dashboard" element={<Dashboard />} />
          ...
          <Route path="/admin" element={<ProtectedAdminRoute><SuperAdmin /></ProtectedAdminRoute>} />
      </Routes>
      </div>
      {!hideFooter && <Footer />}
    </div>
